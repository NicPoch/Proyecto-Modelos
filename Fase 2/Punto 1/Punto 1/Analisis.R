...
#Estados
S_sistema<-c()
for(s_123 in 0:c_123)
{
  for(s_policia in 0:c_policia)
  {
    for(s_uri in 0:c_uri)
    {
...
tiempoParaOcupado<-ExpectedTime(CMTC,1,90)
...
for(s in S_sistema)
{
  if(as.numeric(strsplit(X_n1,split=",")[[1]][2])==c_policia)
  {
    ans<-ans+M_t[1,s][[1]]
  }
}
